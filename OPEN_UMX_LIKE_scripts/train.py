...
def get_statistics(args, encoder, dataset):
    ...
    # Saving the objects:
    # with open('dataset_stats.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    #     pickle.dump([scaler_mean,std], f)


    # # Getting back the objects:
    # with open('dataset_stats.pkl') as f:  # Python 3: open(..., 'rb')
    #     scaler_mean, std = pickle.load(f)    
...
def main():
    ...
    if args.model:
        ...
        # params_path = "/home/nnanos/OPEN_UMX_LIKE_scripts/params.json"
        # with open(params_path, 'r') as stream:
        #     model_params = json.load(stream)
        # unmix = mdn.MDenseNet(**model_params).to(device).float()              


        unmix = model.UNet().to(device).float()
